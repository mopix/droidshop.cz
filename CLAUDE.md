...
- Stojí jádro (tenancy, moduly, kernel služby, superadmin), moduly `categories` + `products` + `shipping` + `customers` + `checkout` + `orders` a veřejný storefront katalogu (Blade SSR, SEO výstupy, 301/410). Zákazník projde nákup od katalogu po děkovnou stránku bez JS; nájemce vidí, edituje a stornuje objednávky v adminu. Chybí online platební brána (vlna 1.4) a faktury/doklady (vlna 1.5).
