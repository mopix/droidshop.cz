...
> **Stav: hotovo (2026-08-09, v0.42.0).** As-is: [`docs/as-is/2026-08-09-dph-a-cas.md`](../../as-is/2026-08-09-dph-a-cas.md).
...
- [x] **Step 2: Implementuj.** `ShopClock::format(DateTimeInterface, bool $withTime)` čte nastavení jednou; volající nikde nesestavuje formát sám.
- [x] **Step 3: Ověř** — `php artisan test tests/Feature/Shop tests/Feature/Modules/Orders --compact`
- [x] **Step 4: Commit** — `feat(shop): use the tenant's time zone and date format`
...
- [x] **Step 2: Implementuj.** Tenká služba nad `TenantContext`. Důvod, proč nestačí `$tenant->vat_payer` v šablonách: volajících bude přes deset a jedenáctý se zapomene (stejná úvaha jako u observeru page cache).
- [x] **Step 3: Commit** — `feat(tax): add one place that answers whether the shop charges VAT`
...
- [x] **Step 2: Implementuj.** Nad existující tabulkou, žádná migrace. `TaxRates::flush()` po každém zápisu.
- [x] **Step 3: Ověř + `npm run build`**
- [x] **Step 4: Commit** — `feat(platform): let the superadmin manage VAT rates`
...
- [x] **Step 2: Implementuj.** Převod v FormRequestu přes `TaxRate::gross()`, nikdy v JS — JS smí jen napovídat, stejné pravidlo jako u variant (vlna 2.4).
- [x] **Step 3: Ověř + `npm run build`**
- [x] **Step 4: Commit** — `feat(products): let a VAT payer enter the price without VAT, and hide VAT from everyone else`
...
- [x] **Step 2: Ověř + commit** — `feat(storefront): stop telling a non-VAT-payer's customers about VAT`
...
- [x] **Step 2: E2E** — přepnout demo na neplátce, projít detail produktu a pokladnu, vrátit zpět.
- [x] **Step 3: Ruční průchod na demu.**
- [x] **Step 4:** as-is, STATUS, CLAUDE.md, `VERSION` → `0.42.0`, CHANGELOG, merge.
