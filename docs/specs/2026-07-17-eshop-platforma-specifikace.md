...
- **Storefront:** Blade šablony renderované serverem (SEO, rychlost, jednoduchost cachování) + Alpine.js/Vue ostrůvky (košík). Headless API pro storefront až post-MVP.
...
- **Page cache storefrontu:** celé HTML pro anonymní GET bez košíku (cookie `has_cart` vypíná cache), klíč `page:{tenant}:{path}:{qs-hash}`, TTL 10 min + eventová invalidace (product.updated → invalidace tagů `product:{id}`, `category:{id}`, `home`). Košík/checkout nikdy necachovat.
...
- Přesun/přejmenování → automatický zápis do `redirects` (staré URL → nové, 301) pro kategorii i všechny produkty s primární kategorií v podstromu.
- Storefront URL schéma: `/kategorie/{slug}` a produkt `/{category-slug}/{product-slug}` (primární kategorie); rozhodnutí zafixovat v MVP — pozdější změna schématu je drahá.
