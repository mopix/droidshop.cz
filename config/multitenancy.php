...
return [
    ...
    'switch_tenant_tasks' => [
        /*
         * Namespaces every cache key with the tenant, so a key written while
         * tenant A is current is unreachable while tenant B is current.
         */
        PrefixCacheTask::class,

        /*
         * SwitchTenantDatabaseTask stays off on purpose. DroidShop runs a
         * shared database with a tenant_id column (spec §4.2 variant B), so
         * there is no per-tenant connection to switch. Isolation is enforced
         * by the BelongsToTenant global scope instead.
         */
    ],
