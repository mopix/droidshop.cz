...
- **Stav:** open — mimo rozsah aktuální vlny (server se v této vlně nemění, viz task brief). Oprava: v `isSafeUrl()` po `str_starts_with($url, '/')` přidat stejnou výjimku jako `BlockUrl::isSafe` (`&& ! str_starts_with($url, '//') && ! str_starts_with($url, '/\\')`), doplnit test do `tests/Unit/Core/Html/HtmlSanitizerTest.php` (pokud existuje) nebo `tests/Feature/...` pokrývající sanitizaci.
