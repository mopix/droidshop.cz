...
@section('content')
    ...
    <div class="grid gap-8 lg:grid-cols-2">
        ...
        <div data-gallery class="card p-4">
            ...
            {{-- Enhanced into a lightbox once JavaScript is there; without it
                 the thumbnails below are still links to the full pictures. --}}
            <a href="{{ $main ? app(\App\Core\Storage\FileStorage::class)->publicUrl($main->path) : '#' }}"
               data-gallery-open
               class="btn btn-outline mt-3 w-full">Zobrazit galerii</a>
        ...
        <div>
            ...
            <div class="card mt-4 p-5">
                ...
                @if ($cartEnabled && $isAvailable)
                    ...
                    --}}
                    <form id="nakup" method="POST" action="{{ route('storefront.checkout.add') }}" class="mt-5">
                        @csrf
    ...
    {{-- Anchors, not tabs: each section is on this page, so a link to it is
         the honest control — and it works with no JavaScript at all. --}}
    <nav class="mt-10 flex flex-wrap gap-x-6 gap-y-2 border-b border-line pb-3 text-sm" aria-label="Sekce produktu">
        @if ($product->description)
            <a href="#nadpis-popis" class="font-medium text-ink hover:underline">Popis produktu</a>
        @endif
        @if ($product->hasDimensions() || $product->weight_g > 0)
            <a href="#nadpis-parametry" class="font-medium text-ink hover:underline">Parametry</a>
        @endif
    </nav>

    @if ($product->description)
        <section class="card prose-shop mt-6 p-6" aria-labelledby="nadpis-popis">
    ...
    @endif
    @if ($cartEnabled && $isAvailable)
        {{-- Shown only once the real button has scrolled away, and only when
             JavaScript is there to notice — it starts hidden, so nobody
             without it gets a strip covering the page. The button scrolls to
             the form rather than posting on its own: one purchase path, not
             two. --}}
        <div data-sticky-buy="#nakup"
             class="hidden fixed inset-x-0 bottom-0 z-30 border-t border-line bg-surface/95 backdrop-blur">
            <div class="shop-container flex items-center justify-between gap-4 py-3">
                <span class="truncate text-sm font-medium text-ink">{{ $product->name }}</span>
                <span class="flex items-center gap-3">
                    <span class="font-bold text-ink">{{ $displayPrice->format() }}</span>
                    <a href="#nakup" class="btn btn-primary">Do košíku</a>
                </span>
            </div>
        </div>
    @endif
